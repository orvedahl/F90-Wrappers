...
module relax_mpi_mod

contains

subroutine relax_mpi(nx, ny, ng, bc_lo_x, bc_hi_x, bc_lo_y, bc_hi_y, &
                     xmin, xmax, ymin, ymax, nsmooth, comm)
  ...
  double precision, intent(in) :: bc_lo_x
  double precision, intent(in) :: bc_hi_x
  double precision, intent(in) :: bc_lo_y
  double precision, intent(in) :: bc_hi_y
...
contains
  ...
  subroutine fillGC(ilo, ihi, jlo, jhi, ng, &
    ...
    if (isr == 1) then
       ...
       if (mype > 0) then
          ...
          !              mype-1, mype, comm, ierr)
          call parallel_send_dv(v(ilo,:), ny+2*ng, mype-1, mype, comm)
       endif

       if (mype < nprocs-1) then
          !call MPI_Recv(v(ihi+1,:), ny+2*ng, MPI_DOUBLE_PRECISION, &
          !              mype+1, mype+1, comm, status, ierr)
          call parallel_recv_dv(v(ihi+1,:), ny+2*ng, mype+1,mype+1, &
                                comm, status)
       endif
       ...
       if (mype < nprocs-1) then
          ...
          !              mype+1, mype, comm, ierr)
          call parallel_send_dv(v(ihi,:), ny+2*ng, mype+1, mype, comm)
       endif

       if (mype > 0) then
          !call MPI_Recv(v(ilo-1,:), ny+2*ng, MPI_DOUBLE_PRECISION, &
          !              mype-1, mype-1, comm, status, ierr)
          call parallel_recv_dv(v(ilo-1,:), ny+2*ng, mype-1,mype-1, &
                                comm, status)
       endif
  ...
  function error(ilo, ihi, jlo, jhi, ng, dx, dy, v)
    ...
    !                   MPI_SUM, comm, ierr)
    call parallel_reduce_d(error, errlocal, MPI_SUM, comm=comm)
...
end subroutine relax_mpi

end module relax_mpi_mod
