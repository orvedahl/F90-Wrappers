...
module parallel
  ...
  integer, parameter :: MPI_CHARACTER        = 1000
  integer, parameter :: MPI_INTEGER          = 1001
  integer, parameter :: MPI_REAL             = 1002
  integer, parameter :: MPI_DOUBLE_PRECISION = 1003
  integer, parameter :: MPI_COMPLEX          = 1004
  integer, parameter :: MPI_LOGICAL          = 1005
  integer, parameter :: MPI_BYTE             = 1006
  integer, parameter :: MPI_PACKED           = 1007
...
contains
  ...
  subroutine sys_abort()
    stop ! this is the non-MPI code, so should be fine
  end subroutine sys_abort

  subroutine wall_second(s) ! return time in seconds
    use bl_types
    real(kind=dp_t), intent(out) :: s
    integer :: t(8)

    call date_and_time(values=t)
    !   hours              minutes          sec    milliseconds
    s = t(5)*3600.0_dp_t + t(6)*60.0_dp_t + t(7) + t(8)*0.001_dp_t
  end subroutine wall_second

  subroutine wall_second_tick(s) ! return seconds per tick of clock
    use bl_types
    real(kind=dp_t), intent(out) :: s
    integer :: cnt_rate

    call system_clock(count_rate=cnt_rate)
    s = 1.0_dp_t / real(cnt_rate,kind=dp_t)
