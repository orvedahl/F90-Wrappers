...
contains
  ...
  subroutine eos_init(small_temp, small_dens, gamma_in)
    ...
    if (parallel_IOProcessor()) print *, 'Initializing MESA EOS...'
...
! a generic wrapper for the MESA version of the Helmholtz based
! electron/positron degenerate EOS.
!
...
! call the eos
       call MESA_eos_from_MAESTRO( &
                   xmass, den_row, temp_row, abar_row, zbar_row, &
                   etot_row, ptot_row, &
...
!---------------------------------------------------------------------------
       ...
       do iter = 1, max_newton
          ...
          if (do_eos_diag) print*,'DERIVS wrt T ',iter, dpt_row,det_row
